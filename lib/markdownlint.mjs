...
function newResults(ruleList) {
  ...
  function toString(useAlias) {
    // eslint-disable-next-line consistent-this, unicorn/no-this-assignment, unicorn/no-this-outside-of-class
...
function lintInput(options, synchronous, callback) {
  ...
  function lintWorker() {
    ...
    if (done) {
      ...
      Object.defineProperty(sortedResults, "toString", { "value": results.toString });
      // Deprecated: Convert results to specified resultVersion
      let convertedResults = sortedResults;
      if (resultVersion === 0) {
        convertedResults = helpers.convertToResultVersion0(sortedResults);
      } else if (resultVersion === 1) {
        convertedResults = helpers.convertToResultVersion1(sortedResults);
      } else if (resultVersion === 2) {
        convertedResults = helpers.convertToResultVersion2(sortedResults);
      }
